    ...
    def _analyze_node(self, node: ast.AST) -> Optional[Dict]:
        ...
        if isinstance(node, ast.Return):
            ...
            if isinstance(node.func, ast.Attribute):
                ...
                if method_name in ['verify_password', 'authenticate', 'validate', 'save', 'delete']:
                    return {
                        'line_number': getattr(node, 'lineno', 0),
                        'content': ast.unparse(node),
                        'importance': 0.80,
...
class JavaCriticalAnalyzer(LanguageAnalyzer):
    ...
    def find_critical_nodes(self, tree: ast.AST) -> List[Dict]:
        ...
        patterns = [
            ...
            (r'public\s+(?:static\s+)?(?:final\s+)?(?:class|interface|enum)\s+\w+', 0.80, 'class_definition'),
            (r'public\s+(?:static\s+)?(?:final\s+)?(?:abstract\s+)?\w+\s+\w+\s*\([^)]*\)', 0.80, 'method_definition'),
...
class CSharpCriticalAnalyzer(LanguageAnalyzer):
    ...
    def find_critical_nodes(self, tree: ast.AST) -> List[Dict]:
        ...
        patterns = [
            ...
            (r'while\s*\([^)]*\)\s*{?', 0.85, 'loop_structure'),
            (r'public\s+(?:static\s+)?(?:sealed\s+)?(?:abstract\s+)?(?:class|interface|struct|enum)\s+\w+', 0.80, 'class_definition'),
            (r'public\s+(?:static\s+)?(?:virtual\s+)?(?:abstract\s+)?(?:override\s+)?\w+\s+\w+\s*\([^)]*\)', 0.80, 'method_definition'),
            (r'throw\s+new\s+\w+Exception[^;]*;', 0.85, 'error_handling'),
            (r'try\s*{', 0.80, 'try_block'),
            (r'catch\s*\([^)]+\)\s*{', 0.80, 'catch_block'),
            (r'finally\s*{', 0.75, 'finally_block'),
...
class CriticalLineExtractor:
    ...
    def extract_critical_lines(self, symbol_content: str, language: str = 'python') -> List[CriticalLine]:
    ...
    def get_implementation_essence(self, symbol_content: str, language: str = 'python') -> str:
    ...
    def rank_line_importance(self, lines: List[str], symbol_type: str = 'function') -> List[float]:
