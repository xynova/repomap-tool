...
class ContextSelector:
    ...
    def select_optimal_context(
        ...
        # Determine strategy
        if strategy is None:
            strategy = self._determine_optimal_strategy(all_symbols, user_context, token_budget)
        ...
        if strategy == SelectionStrategy.BREADTH_FIRST:
            ...
            selected_symbols = self._centrality_based_selection(all_symbols, token_budget)
        else:  # HYBRID
    ...
    def _determine_optimal_strategy(
        ...
        if user_context:
            ...
            if any(word in intent for word in ['overview', 'summary', 'list', 'files']):
                return SelectionStrategy.BREADTH_FIRST
            elif any(word in intent for word in ['detail', 'implementation', 'code', 'function']):
                return SelectionStrategy.DEPTH_FIRST
            elif any(word in intent for word in ['dependency', 'impact', 'relationship']):
                return SelectionStrategy.CENTRALITY_BASED
        
        # Strategy based on symbol characteristics
        high_centrality_count = sum(1 for s in symbols if s.get('centrality_score', 0) > 0.8)
    ...
    def _breadth_first_selection(
        ...
        selected = []
        current_tokens = 0
        tokens_per_symbol = token_budget // min(len(symbols), 20)  # Aim for at least 20 symbols
    ...
    def _depth_first_selection(
        ...
        for symbol in sorted_symbols[:10]:  # Limit to top 10
            ...
            if current_tokens + symbol_tokens <= token_budget:
                ...
                medium_tokens = self._estimate_symbol_tokens(medium_symbol, detail_level='medium')
    ...
    def _centrality_based_selection(
        ...
        sorted_symbols = sorted(
            symbols, 
            key=lambda s: s.get('centrality_score', 0), 
            reverse=True
        )
        ...
        high_centrality = [s for s in sorted_symbols if s.get('centrality_score', 0) > 0.7]
        medium_centrality = [s for s in sorted_symbols if 0.4 <= s.get('centrality_score', 0) <= 0.7]
        low_centrality = [s for s in sorted_symbols if s.get('centrality_score', 0) < 0.4]
        ...
        selected.extend(self._select_within_budget(high_centrality, high_budget, 'full'))
        selected.extend(self._select_within_budget(medium_centrality, medium_budget, 'medium'))
        selected.extend(self._select_within_budget(low_centrality, low_budget, 'minimal'))
        ...
        if depth_budget > 0:
            ...
            for symbol in breadth_symbols:
                if symbol.get('name') in depth_symbol_names:
                    ...
                    detailed = next(s for s in depth_symbols if s.get('name') == symbol.get('name'))
    ...
    def _create_minimal_symbol(self, symbol: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'critical_lines': symbol.get('critical_lines', [])[:1] if symbol.get('critical_lines') else [],
            'signature': symbol.get('signature', ''),
            'dependencies': symbol.get('dependencies', [])[:2] if symbol.get('dependencies') else [],
        }
    ...
    def _create_medium_symbol(self, symbol: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'critical_lines': symbol.get('critical_lines', [])[:2] if symbol.get('critical_lines') else [],
            'signature': symbol.get('signature', ''),
            'dependencies': symbol.get('dependencies', [])[:3] if symbol.get('dependencies') else [],
            'usage_examples': symbol.get('usage_examples', [])[:1] if symbol.get('usage_examples') else [],
        }
    ...
    def _create_detailed_symbol(self, symbol: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'critical_lines': symbol.get('critical_lines', [])[:3] if symbol.get('critical_lines') else [],
            'signature': symbol.get('signature', ''),
            'dependencies': symbol.get('dependencies', [])[:5] if symbol.get('dependencies') else [],
            'usage_examples': symbol.get('usage_examples', [])[:2] if symbol.get('usage_examples') else [],
            'error_cases': symbol.get('error_cases', [])[:2] if symbol.get('error_cases') else [],
            'complexity_score': symbol.get('complexity_score', 0),
        }
    
    def _estimate_symbol_tokens(self, symbol: Dict[str, Any], detail_level: str = 'medium') -> int:
    ...
    def _calculate_priority_breakdown(self, symbols: List[Dict[str, Any]]) -> Dict[str, int]:
