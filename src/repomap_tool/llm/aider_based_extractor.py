...
class AiderBasedExtractor:
    ...
    def extract_critical_lines(self, file_path: str, symbol_content: str = "") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Extracted {len(critical_lines)} critical lines from {file_path}")
...
class CriticalLineExtractor:
    ...
    def extract_critical_lines(
        ...
        if (self.aider_extractor and 
            isinstance(symbol_content, str) and 
            Path(symbol_content).exists()):
    ...
    def _fallback_extraction(self, content: str) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines):
            ...
            if any(pattern in line for pattern in ["def ", "class ", "return ", "if ", "import "]):
