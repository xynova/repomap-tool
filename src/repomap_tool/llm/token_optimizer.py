...
class TokenOptimizer:
    ...
    def prioritize_content_by_importance(
        ...
        prioritized = sorted(
            sections, key=lambda x: x.get("importance", 0.0), reverse=True
        )

        # Group by importance level
        high_importance = [s for s in prioritized if s.get("importance", 0.0) > 0.8]
        medium_importance = [
            s for s in prioritized if 0.5 <= s.get("importance", 0.0) <= 0.8
        ]
    ...
    def allocate_token_budget(
        self, content: Dict[str, Any], max_tokens: int
    ) -> BudgetAllocation:
    ...
    def _summarize_repetitive_patterns(self, content: str) -> str:
        ...
        while i < len(lines):
            ...
            if i + 2 < len(lines):
                ...
                if (
                    line.strip() == next_line.strip() == next_next_line.strip()
                    and line.strip()
    ...
    def _emergency_truncation(self, content: str, max_tokens: int, model: str) -> str:
        ...
        for line in lines:
            ...
            if (
                current_tokens + line_tokens <= max_tokens - 20
            ):  # Leave buffer for truncation message
