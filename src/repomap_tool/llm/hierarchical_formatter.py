...
class HierarchicalFormatter:
    ...
    def _format_file_summary(
        self, file_info: Dict[str, Any], is_last: bool
    ) -> List[str]:
    ...
    def format_with_budget_constraints(
        self, content: str, max_lines: int = 50, max_depth: int = 3
    ) -> str:
        ...
        for line in lines:
            if (
                len(truncated_lines) >= max_lines - 2
        ...
        truncated_lines.append(
            f"... truncated at {max_lines} lines, depth {current_depth}"
        )
