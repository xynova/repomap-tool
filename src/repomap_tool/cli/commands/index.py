...
def create(
    ...
    try:
        ...
        if hasattr(repomap, 'embedding_matcher') and repomap.embedding_matcher and repomap.embedding_matcher.enabled:
            ...
            if identifiers_with_files:
                repomap.embedding_matcher.batch_compute_embeddings(identifiers_with_files)
                console.print(f"[green]✓ Cached embeddings for {len(identifiers_with_files)} identifiers[/green]")
            else:
                console.print("[yellow]No identifiers found for embedding computation[/yellow]")
