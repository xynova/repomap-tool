...
class ImpactAnalyzer:
    ...
    def analyze_change_impact(self, changed_files: List[str]) -> ImpactReport:
        ...
        try:
            ...
            risk_score = self._calculate_overall_risk_score(changed_files, affected_files)
            
            # Assess breaking change potential
            breaking_change_potential = self._assess_breaking_change_potential(changed_files)
            ...
            logger.info(f"Impact analysis complete: {len(affected_files)} files affected, risk score: {risk_score:.2f}")
        ...
        try:
            for changed_file in changed_files:
                if changed_file in self.graph.nodes:
                    ...
                    transitive_dependents = self.graph.calculate_transitive_dependents(changed_file)
                    affected_files.update(transitive_dependents)
                    
                    # Also consider function call dependencies
                    if self.graph.call_graph:
                        function_dependents = self.graph._get_file_function_dependents(changed_file)
                        for func_name in function_dependents:
                            if func_name in self.graph.call_graph.function_locations:
                                dependent_file = self.graph.call_graph.function_locations[func_name]
    ...
    def _calculate_overall_risk_score(self, changed_files: List[str], affected_files: Set[str]) -> float:
        ...
        try:
            ...
            total_risk = base_risk + file_count_risk + affected_count_risk + centrality_risk + breaking_change_risk
    ...
    def _assess_breaking_change_potential(self, changed_files: List[str]) -> Dict[str, str]:
        ...
        try:
            ...
            for changed_file in changed_files:
                ...
                if self.graph.call_graph:
                    func_dependents = self.graph._get_file_function_dependents(changed_file)
    ...
    def _suggest_test_files(self, changed_files: List[str], affected_files: Set[str]) -> List[str]:
    ...
    def _generate_impact_summary(self, changed_files: List[str], affected_files: Set[str], 
                                risk_score: float, breaking_change_potential: Dict[str, str]) -> str:
        ...
        try:
            ...
            # Overall impact
            summary_parts.append(f"Changes to {len(changed_files)} file(s) will affect {len(affected_files)} file(s) total.")
            ...
            summary_parts.append(f"Overall risk level: {risk_level} (score: {risk_score:.2f})")
            
            # Breaking change assessment
            high_risk_files = [f for f, risk in breaking_change_potential.items() if risk == "HIGH"]
            if high_risk_files:
                summary_parts.append(f"High breaking change potential in {len(high_risk_files)} file(s).")
            
            # Recommendations
            if risk_score > 0.7:
                summary_parts.append("Recommend comprehensive testing and careful review.")
            elif risk_score > 0.4:
                summary_parts.append("Recommend focused testing of affected areas.")
            else:
                summary_parts.append("Low risk changes, standard testing should suffice.")
    ...
    def get_cached_impact_report(self, changed_files: List[str]) -> Optional[ImpactReport]:
