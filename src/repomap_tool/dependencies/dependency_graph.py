...
class DependencyGraph:
    ...
    def _validate_files(self, project_files: List[str]) -> None:
        ...
        if len(valid_files) != len(project_files):
            logger.warning(f"Filtered {len(project_files) - len(valid_files)} invalid files")
    ...
    def _add_edges(self) -> None:
        ...
        project_imports = self.import_analyzer.analyze_project_imports(list(self.nodes.keys()))
    ...
    def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                'average_in_degree': sum(self.graph.in_degree(node) for node in self.graph.nodes()) / max(len(self.nodes), 1),
                'average_out_degree': sum(self.graph.out_degree(node) for node in self.graph.nodes()) / max(len(self.nodes), 1),
            }
    ...
    def import_from_networkx(self, graph: nx.DiGraph) -> None:
        ...
        try:
            ...
            logger.info(f"Imported graph with {len(self.nodes)} nodes and {len(self.graph.edges)} edges")
