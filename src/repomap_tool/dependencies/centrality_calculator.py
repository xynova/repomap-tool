...
class CentralityCalculator:
    ...
    def calculate_betweenness_centrality(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Calculated betweenness centrality for {len(betweenness_scores)} files")
    ...
    def calculate_pagerank_centrality(self, alpha: float = 0.85, max_iter: int = 100) -> Dict[str, float]:
        ...
        try:
            ...
            pagerank_scores = nx.pagerank(self.graph.graph, alpha=alpha, max_iter=max_iter)
            ...
            logger.info(f"Calculated PageRank centrality for {len(pagerank_scores)} files")
    ...
    def calculate_eigenvector_centrality(self) -> Dict[str, float]:
        ...
        try:
            ...
            eigenvector_scores = nx.eigenvector_centrality(self.graph.graph, max_iter=1000)
            ...
            logger.info(f"Calculated eigenvector centrality for {len(eigenvector_scores)} files")
    ...
    def calculate_closeness_centrality(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Calculated closeness centrality for {len(closeness_scores)} files")
    ...
    def calculate_composite_importance(self, weights: Optional[Dict[str, float]] = None) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Calculated composite importance for {len(composite_scores)} files")
    ...
    def get_top_central_files(self, centrality_type: str = "composite", top_n: int = 10) -> List[Tuple[str, float]]:
    ...
    def get_centrality_ranking(self, centrality_type: str = "composite") -> List[Tuple[str, float, int]]:
        ...
        try:
            ...
            sorted_files = sorted(scores.items(), key=lambda x: x[1], reverse=True)
            ranking = [(file_path, score, rank + 1) for rank, (file_path, score) in enumerate(sorted_files)]
    ...
    def get_centrality_percentile(self, file_path: str, centrality_type: str = "composite") -> float:
    ...
    def _validate_centrality_scores(self, scores: Dict[str, float]) -> None:
        ...
        if min_score < 0 or max_score > 1:
            logger.warning(f"Centrality scores outside expected range [0,1]: min={min_score}, max={max_score}")
        
        # Check for all-zero or all-one scores (likely calculation error)
        unique_scores = set(scores.values())
        if len(unique_scores) == 1:
            logger.warning(f"All centrality scores are identical: {list(unique_scores)[0]}")
