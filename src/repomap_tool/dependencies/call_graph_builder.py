...
class PythonCallAnalyzer(CallAnalyzer):
    ...
    def _analyze_call_node(self, node: ast.Call, file_path: str) -> Optional[FunctionCall]:
...
class JavaScriptCallAnalyzer(CallAnalyzer):
    ...
    def _find_calling_function(self, function_lines: Dict[int, str], call_line: int) -> Optional[str]:
        """Find the function that contains a call at the given line."""
        try:
            # Find the function declaration that comes before this line
            candidate_functions = [
                (line, func) for line, func in function_lines.items() 
                if line <= call_line
            ]
...
class CallGraphBuilder:
    ...
    def __init__(self):
        ...
        logger.info(f"CallGraphBuilder initialized with {len(self.language_analyzers)} language analyzers")
    
    def build_call_graph(self, project_files: List[str], max_workers: int = 4) -> CallGraph:
        ...
        logger.info(f"Call graph built: {len(all_calls)} function calls, {len(function_locations)} functions")
        return call_graph
    ...
    def resolve_cross_file_calls(self, calls: List[FunctionCall], project_files: List[str]) -> List[FunctionCall]:
    ...
    def get_call_statistics(self, call_graph: CallGraph) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                'unique_callers': len(set(call.caller for call in call_graph.function_calls)),
                'unique_callees': len(set(call.callee for call in call_graph.function_calls)),
                'method_calls': len([call for call in call_graph.function_calls if call.is_method_call]),
                'function_calls': len([call for call in call_graph.function_calls if not call.is_method_call]),
            }
            ...
            if callee_counts:
                stats['most_called_function'] = max(callee_counts.items(), key=lambda x: x[1])
                stats['average_calls_per_function'] = sum(callee_counts.values()) / len(callee_counts)
            
            # File distribution
            file_call_counts = {}
            for call in call_graph.function_calls:
                file_call_counts[call.file_path] = file_call_counts.get(call.file_path, 0) + 1
    ...
    def find_function_dependencies(self, call_graph: CallGraph, function_name: str) -> List[str]:
    ...
    def find_function_dependents(self, call_graph: CallGraph, function_name: str) -> List[str]:
