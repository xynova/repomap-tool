...
class AdvancedDependencyGraph(DependencyGraph):
    ...
    def _build_function_dependency_maps(self) -> None:
        ...
        try:
            ...
            logger.info(f"Built function dependency maps: {len(self.function_dependencies)} functions")
            ...
            for file_path, node in self.nodes.items():
                if file_path in file_functions:
                    ...
                    node.structural_info = {
                        ...
                        'function_dependencies': self._get_file_function_dependencies(file_path),
                        'function_dependents': self._get_file_function_dependents(file_path),
                    }
    ...
    def calculate_transitive_dependencies(self, file_path: str, max_depth: int = 10) -> Set[str]:
    ...
    def calculate_transitive_dependents(self, file_path: str, max_depth: int = 10) -> Set[str]:
        ...
        while queue:
            ...
            if self.call_graph:
                function_dependencies = self._get_file_function_dependencies(current_file)
    ...
    def _calculate_path_depth(self, start_file: str, end_file: str, max_depth: int = 20) -> int:
    ...
    def find_dependency_clusters(self) -> List[List[str]]:
        ...
        try:
            ...
            enhanced_clusters = self._enhance_clusters_with_function_calls(import_clusters)
    ...
    def _enhance_clusters_with_function_calls(self, import_clusters: List[List[str]]) -> List[List[str]]:
                        ...
                        if func_name in self.function_dependents:
                            for caller in self.function_dependents[func_name]:
                                if caller in self.call_graph.function_locations:
                                    caller_file = self.call_graph.function_locations[caller]
                        ...
                        if func_name in self.function_dependencies:
                            for callee in self.function_dependencies[func_name]:
                                if callee in self.call_graph.function_locations:
                                    callee_file = self.call_graph.function_locations[callee]
        ...
        try:
            ...
            for file_path, node in self.nodes.items():
                ...
                coupling_metrics[file_path] = {
                    ...
                    'instability': len(efferent_coupling) / max(len(afferent_coupling) + len(efferent_coupling), 1)
                }
    ...
    def identify_hotspots(self) -> List[str]:
        ...
        try:
            ...
            hotspots.sort(key=lambda x: len(self.get_dependents(x)) + len(self._get_file_function_dependents(x)), reverse=True)
    ...
    def get_enhanced_graph_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.call_graph:
                call_stats = self.call_graph_builder.get_call_statistics(self.call_graph)
                base_stats.update({
                    ...
                    'files_with_functions': len([n for n in self.nodes.values() if n.functions])
                })
            
            # Add advanced metrics
            base_stats.update({
                'hotspots': len(self.identify_hotspots()),
                'refactoring_opportunities': len(self.suggest_refactoring_opportunities()),
