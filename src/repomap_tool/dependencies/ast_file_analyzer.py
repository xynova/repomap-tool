...
class ASTFileAnalyzer:
    ...
    def analyze_file(
        ...
        try:
            ...
            if file_ext == '.py':
                ...
                result = self._analyze_javascript_typescript(content, file_path, analysis_type)
            else:
                # For other file types, return empty result with analysis error
                logger.debug(f"Skipping analysis for unsupported file type: {file_path}")
    ...
    def _analyze_javascript_typescript(
        ...
        try:
            ...
            function_pattern = r'(?:export\s+)?(?:async\s+)?function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\('
            ...
            for match in re.finditer(method_pattern, content):
                ...
                if method_name not in ['constructor', 'if', 'for', 'while', 'switch', 'catch', 'with', 'try']:
            ...
            for match in re.finditer(call_pattern, content):
                ...
                if func_name not in ['if', 'for', 'while', 'switch', 'catch', 'with']:  # Skip control structures
            ...
            for match in re.finditer(named_import_pattern, content, re.DOTALL):
                ...
                imports.append(Import(
                    ...
                    import_type=ImportType.RELATIVE if module.startswith('.') else ImportType.ABSOLUTE,
                    line_number=self._get_line_number(content, match.start())
                ))
            ...
            for match in re.finditer(require_pattern, content):
                imports.append(Import(
                    ...
                    import_type=ImportType.RELATIVE if match.group(1).startswith('.') else ImportType.ABSOLUTE,
                    line_number=self._get_line_number(content, match.start())
                ))
