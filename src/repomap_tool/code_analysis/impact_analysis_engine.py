...
class ImpactAnalysisEngine:
    ...
    def analyze_file_impact(
        ...
        graph_to_use = (
            dependency_graph if dependency_graph is not None else self.dependency_graph
        )
        reverse_dependencies = self._analyze_reverse_dependencies(
            file_path, all_files, graph_to_use
        )

        # Function call analysis
        function_call_analysis = self._analyze_function_calls(
            ast_result.function_calls, ast_result.imports
        )
    ...
    def _analyze_function_calls(
        self, function_calls: List[Any], imports: Optional[List[Any]] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _determine_function_source(
        self, func_call: Any, import_map: Optional[Dict[str, str]] = None
    ) -> str:
    ...
    def _calculate_impact_score(self, ast_result: FileAnalysisResult) -> float:
        ...
        try:
            ...
            definition_score = min(
                (defined_functions + defined_classes * 2) / 20.0, 0.3
            )  # Classes weighted more
