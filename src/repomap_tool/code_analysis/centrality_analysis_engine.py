...
class CentralityAnalysisEngine:
    ...
    def analyze_file_centrality(
        ...
        """
        try:
            # Calculate centrality score and ranking using absolute path
            ...
            centrality_breakdown = self._calculate_centrality_breakdown(file_path)
    ...
    def _calculate_centrality_metrics(self, file_path: str) -> tuple[float, int, int]:
    ...
    def _analyze_dependencies(
        ...
        if (
            self.dependency_graph
            and hasattr(self.dependency_graph, "nodes")
            and file_path in self.dependency_graph.nodes
        ):
            node = self.dependency_graph.nodes[file_path]
            ...
            reverse_dep_list = [
                rel.target_file for rel in reverse_dep_relationships
            ]  # Extract file paths from relationships
    ...
    def _calculate_centrality_breakdown(
        ...
        if not self.centrality_calculator:
            logger.warning(f"Centrality calculator is None for {file_path}")
            return None
        ...
        try:
            ...
            if degree_scores and file_path in degree_scores:
                centrality_breakdown["degree_centrality"] = degree_scores[file_path]
            if betweenness_scores and file_path in betweenness_scores:
                centrality_breakdown["betweenness_centrality"] = betweenness_scores[
                    file_path
                ]
            if pagerank_scores and file_path in pagerank_scores:
                centrality_breakdown["pagerank"] = pagerank_scores[file_path]

            # If no centrality scores were found for this file, return None
            if not centrality_breakdown:
                logger.debug(f"No centrality scores found for {file_path}")
                return None
            ...
            try:
                ...
                if eigenvector_scores and file_path in eigenvector_scores:
                    centrality_breakdown["eigenvector"] = eigenvector_scores[file_path]
            except Exception as e:
            ...
            try:
                ...
                if closeness_scores and file_path in closeness_scores:
                    centrality_breakdown["closeness"] = closeness_scores[file_path]
            except Exception as e:
