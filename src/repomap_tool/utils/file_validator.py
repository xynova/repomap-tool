...
class FileValidator:
        ...
        try:
            ...
            if len(str(path_obj)) > MAX_PATH_LENGTH:
                raise ValidationError(
                    f"Resolved path too long: {len(str(path_obj))} > {MAX_PATH_LENGTH}"
                )
            ...
            if path_obj.exists():
                ...
                raise ValidationError(
                    f"Path does not exist and creation not allowed: {path_obj}"
                )
    ...
    def validate_file_operation(
        self, path: Union[str, Path], operation: str, max_size: Optional[int] = None
    ) -> Path:
        ...
        if path_obj.exists():
            ...
            if file_size > max_allowed:
                raise ValidationError(
                    f"File too large: {file_size} > {max_allowed} bytes"
                )
    ...
    def safe_write_text(
        ...
        if len(content_bytes) > max_allowed:
            raise ValidationError(
                f"Content too large: {len(content_bytes)} > {max_allowed} bytes"
            )
        ...
        try:
            ...
            logger.debug(
                f"Successfully wrote {len(content_bytes)} bytes to {validated_path}"
            )
        except OSError as e:
    ...
    def safe_create_directory(
        ...
        validated_path = self.validate_path(
            path, must_exist=False, must_be_dir=False, allow_create=True
        )
