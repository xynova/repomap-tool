...
class SessionStore:
    ...
    def save_session(self, session: ExplorationSession) -> None:
        ...
        try:
            ...
            raise SessionStoreError(f"Failed to save session {session.session_id}: {e}") from e
    ...
    def load_session(self, session_id: str) -> Optional[ExplorationSession]:
        ...
        try:
            ...
            raise CorruptedSessionError(f"Session {session_id} has corrupted JSON data: {e}") from e
        except Exception as e:
        ...
        try:
            ...
            if session:
                ...
                if session.project_path == project_path:
                    logger.debug(f"Using existing session {session_id} for {project_path}")
                    return session
                else:
                    logger.warning(
            ...
            logger.warning(f"Session {session_id} not found or corrupted, creating new: {e}")
            session = ExplorationSession(session_id=session_id, project_path=project_path)
