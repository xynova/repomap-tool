...
class SpellCheckerService:
    ...
    def __init__(self, custom_dictionary: Optional[Set[str]] = None):
        ...
        if self.codespell_available:
            logger.debug("SpellCheckerService initialized with codespell (programming-optimized)")
        else:
            logger.warning("codespell not available, falling back to basic spellchecking")
    ...
    def _check_codespell_availability(self) -> bool:
        ...
        """
        try:
            result = subprocess.run(['codespell', '--version'], 
    ...
    def _run_codespell(self, text: str) -> List[str]:
        ...
        try:
            ...
            # Run codespell
            result = subprocess.run(['codespell', temp_file], 
        ...
        for suggestion in codespell_suggestions[:max_suggestions]:
            ...
            for i, word in enumerate(words):
                ...
                if clean_word and suggestion != clean_word:
                    ...
                    if partial_correction not in suggestions and partial_correction != query:
