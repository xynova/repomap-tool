...
class EnhancedRepoMapClient:
    ...
    def get_dynamic_repo_map(
        ...
        response = requests.post(f"{self.api_url}/repo-map/dynamic", json=payload, timeout=30)
    ...
    def analyze_context(self, message_text: str) -> Dict[str, Any]:
        ...
        response = requests.post(f"{self.api_url}/context/analyze", json=payload, timeout=30)
        return response.json()  # type: ignore
...
class ConversationManager:
    def __init__(
        self, repo_map_client: EnhancedRepoMapClient, project_path: str
    ) -> None:
    ...
    def update_context(self, message: str) -> Optional[Dict[str, Any]]:
        ...
        if context_analysis["success"]:
            ...
            print(
                f"  Mentioned identifiers: {context_analysis['mentioned_identifiers']}"
            )
        ...
        result = self.repo_map_client.get_dynamic_repo_map(
            project_path=self.project_path, message_text=message
        )
...
def example_conversation() -> None:
    ...
    print("\n=== Message 1: General Question ===")
    context1 = conversation.send_message(
    ...
    print("\n=== Message 2: Specific File Mention ===")
    context2 = conversation.send_message(
        "Can you explain what happens in `src/main.py`?"
    )
    print("Context for LLM:")
    ...
    print("\n=== Message 5: Complex Request ===")
    context5 = conversation.send_message(
...
def openai_integration_example() -> None:
    ...
    for i, message in enumerate(user_messages, 1):
        ...
        conversation.repo_map_client.add_message(
...
def langchain_integration_example() -> None:
    ...
    prompt = PromptTemplate(
        input_variables=["context", "user_request"], template=template
    )
