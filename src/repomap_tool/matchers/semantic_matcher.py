...
class DomainSemanticMatcher:
    ...
    def __init__(self, verbose: bool = True):
        ...
        self.semantic_mappings = {
            ...
            'database': [
                ...
                'constraint', 'foreign', 'primary', 'unique', 'transaction', 'commit',
                'rollback', 'connection', 'pool', 'migration', 'seed'
            ],
            'testing': [
                'test', 'spec', 'mock', 'stub', 'fixture', 'assert', 'verify',
                'check', 'validate', 'expect', 'should', 'describe', 'it',
                'before', 'after', 'setup', 'teardown', 'coverage', 'unit',
                'integration', 'e2e', 'acceptance', 'regression'
            ],
            'file_operations': [
                'file', 'read', 'write', 'open', 'close', 'save', 'load',
                'upload', 'download', 'upload', 'stream', 'buffer', 'path',
                'directory', 'folder', 'create', 'delete', 'move', 'copy',
                'rename', 'exists', 'size', 'type', 'extension'
            ],
            'network': [
                'http', 'https', 'url', 'uri', 'request', 'response', 'client',
                'server', 'socket', 'connection', 'port', 'host', 'domain',
                'ip', 'dns', 'proxy', 'gateway', 'router', 'firewall'
            ],
            'logging': [
                'log', 'logger', 'debug', 'info', 'warn', 'error', 'fatal',
                'trace', 'level', 'output', 'console', 'file', 'syslog',
                'format', 'timestamp', 'message', 'context', 'metadata'
            ],
            'caching': [
                'cache', 'memoize', 'store', 'retrieve', 'get', 'set', 'put',
                'delete', 'clear', 'expire', 'ttl', 'lru', 'redis', 'memcached',
                'invalidate', 'refresh', 'update', 'hit', 'miss'
            ],
            'validation': [
            ...
            'security': [
                'security', 'encrypt', 'decrypt', 'hash', 'salt', 'sign',
                'verify', 'certificate', 'key', 'secret', 'password', 'token',
                'permission', 'role', 'access', 'audit', 'compliance'
            ],
            'performance': [
        ...
        if self.verbose:
            logger.info(f"Initialized DomainSemanticMatcher with {len(self.semantic_mappings)} categories")
    ...
    def find_semantic_matches(self, query: str, all_identifiers: Set[str], 
    ...
    def add_custom_mapping(self, category: str, terms: List[str]):
        ...
        if self.verbose:
            logger.info(f"Added custom mapping for category '{category}' with {len(terms)} terms")
