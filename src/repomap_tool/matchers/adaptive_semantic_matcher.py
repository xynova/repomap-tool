...
class AdaptiveSemanticMatcher:
    ...
    def learn_from_identifiers(self, all_identifiers: Set[str]):
        ...
        if self.verbose:
            logger.info(f"Learned from {self.total_identifiers} identifiers with {len(self.word_frequencies)} unique words")
    ...
    def get_related_identifiers(self, identifier: str, max_results: int = 10) -> List[Tuple[str, float]]:
        """
        ...
        for other_identifier in self.identifier_words.keys():
            if other_identifier != identifier:
                similarity = self.cosine_similarity(identifier_vector, self.get_identifier_vector(other_identifier))
    ...
    def get_semantic_clusters(self, all_identifiers: Set[str], 
                            similarity_threshold: float = 0.3) -> List[Set[str]]:
        """
