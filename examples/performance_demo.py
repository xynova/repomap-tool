...
def display_detailed_metrics(results: list):
    ...
    for result in results:
        ...
        config_table.add_row("Parallel Threshold", str(config.get("parallel_threshold", "N/A")))
        config_table.add_row("Progress Enabled", str(config.get("enable_progress", "N/A")))
        config_table.add_row("Monitoring Enabled", str(config.get("enable_monitoring", "N/A")))
        ...
        if processing_stats:
            ...
            stats_table.add_row("Total Files", str(processing_stats.get("total_files", 0)))
            stats_table.add_row("Successful Files", str(processing_stats.get("successful_files", 0)))
            stats_table.add_row("Failed Files", str(processing_stats.get("failed_files", 0)))
            stats_table.add_row("Success Rate", f"{processing_stats.get('success_rate', 0):.1f}%")
            stats_table.add_row("Total Identifiers", str(processing_stats.get("total_identifiers", 0)))
            stats_table.add_row("Processing Time", f"{processing_stats.get('processing_time', 0):.2f}s")
            stats_table.add_row("Files per Second", f"{processing_stats.get('files_per_second', 0):.1f}")
        ...
        for i in range(0, len(results), 2):
            if i + 1 < len(results):
                ...
                if sequential["success"] and parallel["success"]:
                    improvement = (sequential["total_time"] / parallel["total_time"]) if parallel["total_time"] > 0 else 0
                    console.print(f"• {sequential['label']} vs {parallel['label']}: {improvement:.1f}x faster")
