...
def calculate_tfidf_similarity(query: str, identifier: str, word_frequencies: Dict[str, int], total_identifiers: int) -> float:
...
def hybrid_similarity(query: str, identifier: str, word_frequencies: Dict[str, int], total_identifiers: int) -> float:
    ...
    # TF-IDF similarity
    tfidf_score = calculate_tfidf_similarity(query, identifier, word_frequencies, total_identifiers)
...
def demonstrate_hybrid_vs_rigid():
    ...
    print(f"   Found {len(word_frequencies)} unique words across {total_identifiers} identifiers")
    ...
    for query, description in test_cases:
        ...
        matches = []
        for identifier in sample_identifiers:
            score = hybrid_similarity(query, identifier, word_frequencies, total_identifiers)
    ...
    for word, freq in sorted(word_frequencies.items(), key=lambda x: math.log(total_identifiers / x[1]), reverse=True)[:10]:
        idf = math.log(total_identifiers / freq)
        print(f"   '{word}': {freq} occurrences (IDF: {idf:.2f})")
