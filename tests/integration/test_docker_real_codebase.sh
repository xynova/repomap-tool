...
run_test "Test 1: Analyze Real Codebase" \
    "python3 -m repomap_tool.cli analyze /project --semantic --threshold 0.05 --verbose --output json" || exit 1

# Test 2: Search for specific identifiers in the real codebase
run_test "Test 2: Search for DockerRepoMap Class" \
    "python3 -m repomap_tool.cli search /project 'DockerRepoMap' --match-type hybrid --threshold 0.5 --verbose --output json" || exit 1

# Test 3: Search for function names
run_test "Test 3: Search for analyze_project Function" \
    "python3 -m repomap_tool.cli search /project 'analyze_project' --match-type fuzzy --threshold 0.6 --verbose --output json" || exit 1

# Test 4: Search for matcher classes (fixed query)
run_test "Test 4: Search for FuzzyMatcher Class" \
    "python3 -m repomap_tool.cli search /project 'FuzzyMatcher' --match-type fuzzy --threshold 0.5 --verbose --output json" || exit 1

# Test 5: Search for SemanticMatcher
run_test "Test 5: Search for SemanticMatcher Class" \
    "python3 -m repomap_tool.cli search /project 'SemanticMatcher' --match-type fuzzy --threshold 0.5 --verbose --output json" || exit 1

# Test 6: Search for CLI commands
run_test "Test 6: Search for CLI Commands" \
    "python3 -m repomap_tool.cli search /project 'analyze' --match-type semantic --threshold 0.4 --verbose --output json" || exit 1

# Test 7: Analyze with fuzzy matching
run_test "Test 7: Fuzzy Analysis of Real Codebase" \
    "python3 -m repomap_tool.cli analyze /project --fuzzy --threshold 0.6 --verbose --output json" || exit 1

# Test 8: Analyze with hybrid matching
run_test "Test 8: Hybrid Analysis of Real Codebase" \
    "python3 -m repomap_tool.cli analyze /project --fuzzy --semantic --threshold 0.1 --verbose --output json" || exit 1

# Test 9: Search for configuration related code
run_test "Test 9: Search for Configuration Code" \
    "python3 -m repomap_tool.cli search /project 'config' --match-type semantic --threshold 0.3 --verbose --output json" || exit 1

# Test 10: Search for test related code
run_test "Test 10: Search for Test Code" \
    "python3 -m repomap_tool.cli search /project 'test' --match-type hybrid --threshold 0.4 --verbose --output json" || exit 1

# Test 11: Search for specific function types
run_test "Test 11: Search for Function Definitions" \
    "python3 -m repomap_tool.cli search /project 'def' --match-type fuzzy --threshold 0.5 --verbose --output json" || exit 1

# Test 12: Search for class definitions
run_test "Test 12: Search for Class Definitions" \
    "python3 -m repomap_tool.cli search /project 'class' --match-type fuzzy --threshold 0.5 --verbose --output json" || exit 1
