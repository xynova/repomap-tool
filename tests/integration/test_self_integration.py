...
class TestSelfIntegration:
    ...
    def test_fuzzy_search_independently(self):
        ...
        for query in test_queries:
            ...
            assert len(search_response.results) > 0, f"Fuzzy search should find results for '{query}'"
            ...
            print(f"Fuzzy search for '{query}' found {len(search_response.results)} results")
    ...
    def test_semantic_search_independently(self):
        ...
        for query in test_queries:
            ...
            # Check that we get results
            assert len(search_response.results) > 0, f"Semantic search should find results for '{query}'"
            ...
            print(f"Semantic search for '{query}' found {len(search_response.results)} results")
    ...
    def test_hybrid_search_combination(self):
        ...
        assert len(fuzzy_response.results) > 0, "Fuzzy search should work in hybrid mode"
        ...
        assert len(semantic_response.results) > 0, "Semantic search should work in hybrid mode"
        ...
        print(f"Hybrid search for 'RepoMap' found {len(hybrid_response.results)} results")
        print(f"Fuzzy search for 'RepoMap' found {len(fuzzy_response.results)} results")
        print(f"Semantic search for 'search' found {len(semantic_response.results)} results")
    ...
    def test_search_specific_identifiers(self):
        ...
        for query in specific_queries:
            ...
            assert any(query.lower() in name.lower() for name in found_names), \
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            assert "Query cannot be empty" in str(e), f"Expected empty query error, got: {e}"
    ...
    def test_performance_benchmark(self):
        ...
        for query in test_queries:
            search_request = SearchRequest(query=query, match_type="fuzzy", max_results=10)
        ...
        start_time = time.time()
        for query in test_queries:
            search_request = SearchRequest(query=query, match_type="semantic", max_results=10)
        ...
        start_time = time.time()
        for query in test_queries:
            search_request = SearchRequest(query=query, match_type="hybrid", max_results=10)
