...
""")
    ...
    def test_explore_command_creates_session_and_tree(self, cli_runner, temp_project):
        ...
        assert ("Found" in result.output and "exploration contexts" in result.output) or "No high-confidence entrypoints found" in result.output
...
""")
    ...
    def test_llm_friendly_output_real(self, cli_runner, temp_project):
        ...
        # Test text output
        result_text = cli_runner.invoke(cli, [
            "--no-color",
            "analyze",
            temp_project,
            "--fuzzy",
...
""")
    ...
    def test_show_centrality_command_real(self, cli_runner, temp_project_with_dependencies):
        ...
        result_file = cli_runner.invoke(cli, [
            "--no-color",
            "show-centrality",
            temp_project_with_dependencies,
            "--file", main_file,
        ...
        assert result_file.exit_code in [0, 1]  # Accept both success and "file not found"
        
        # Should contain some output
        output = result_file.output
        assert len(output) > 0

    def test_impact_analysis_command_real(self, cli_runner, temp_project_with_dependencies):
        ...
        result_multi = cli_runner.invoke(cli, [
            "--no-color",
            "impact-analysis",
            temp_project_with_dependencies,
            "--files", main_file, auth_file,
            "--output", "json"
        ])
    ...
    def test_find_cycles_command_real(self, cli_runner, temp_project_with_dependencies):
        ...
        output = result.output
        assert "No circular dependencies found" in output or "Circular Dependencies Found" in output
    ...
    def test_cache_command_real(self, cli_runner, temp_project_with_dependencies):
        ...
        # Test cache refresh
        result_refresh = cli_runner.invoke(cli, [
            "--no-color",
            "cache",
            temp_project_with_dependencies,
            "--refresh",
        ...
        # Test verbose mode
        result_verbose = cli_runner.invoke(cli, [
            "--no-color",
            "cache",
            temp_project_with_dependencies,
            "--verbose",
...
'''
    ...
    def malformed_project(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (src_dir / "syntax_error.py").write_text("def broken_function(\n    # Missing closing parenthesis\n    pass")
            (src_dir / "import_error.py").write_text("import nonexistent_module")
            (src_dir / "valid.py").write_text("def valid_function():\n    return 'valid'")
    ...
    def permission_denied_project(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            restricted_file.write_text("def restricted_function():\n    return 'restricted'")
...
""")
    ...
    def test_analyze_command_edge_cases(self, cli_runner, empty_project, large_project, malformed_project):
    ...
    def test_analyze_command_invalid_parameters(self, cli_runner, temp_project):
        ...
        result = cli_runner.invoke(cli, [
            ...
            "analyze",
            temp_project,
            "--threshold", "2.0",  # Invalid: should be 0.0-1.0
            "--output", "json"
        ])
    ...
    def test_dependency_analysis_edge_cases(self, cli_runner, empty_project, large_project):
    ...
    def test_performance_command_edge_cases(self, cli_runner, empty_project, large_project):
