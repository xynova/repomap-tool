...
class TestQuerySyntaxValidation:
    ...
    def test_all_query_files_have_valid_syntax(self, query_loader, supported_languages):
        ...
        for language in supported_languages:
            try:
                ...
                try:
                    ...
                    error_msg = (
                        f"Language '{language}': Could not get language parser: {e}"
                    )
                    if language in critical_languages:
                        errors.append(error_msg)
                    else:
                ...
                try:
                    ...
                    assert (
                        query is not None
                    ), f"Language '{language}': Query compilation returned None"
                    ...
                    if hasattr(e, "row") and hasattr(e, "column"):
                        ...
                        full_error = (
                            f"Language '{language}': Query syntax error: {error_msg}"
                        )
                    ...
                    error_msg = (
                        f"Language '{language}': Unexpected error compiling query: {e}"
                    )
                    if language in critical_languages:
                        errors.append(error_msg)
                    else:
        ...
        if errors:
            ...
            pytest.fail(
                f"Query syntax validation failed for {len(errors)} critical language(s):\n{error_msg}"
            )
    ...
    def test_specific_languages_have_queries(self, query_loader):
        ...
        if missing:
            pytest.fail(
                f"Missing query files for critical languages: {', '.join(missing)}"
            )
    ...
    def test_query_file_exists_for_language(self, query_loader, language):
        ...
        ), f"Query file not found for language: {language}"
        ...
        try:
            ...
            if language_obj is not None:
                ...
                assert (
                    query is not None
                ), f"Query compilation returned None for language: {language}"
        except Exception:
