    ...
    def test_analyze_command_cli_overrides_config_file(self):
        ...
        try:
            ...
            with patch('src.repomap_tool.cli.DockerRepoMap') as mock_repo_map:
                ...
                mock_instance.analyze_project_with_progress.return_value = mock_project_info
                mock_repo_map.return_value = mock_instance
                ...
                result = self.runner.invoke(cli, [
                    ...
                    '--threshold', '0.8',  # Override: change threshold
                    '--max-results', '100',  # Override: change max results
                    '--output', 'table',  # Override: change output format
                    '--verbose',  # Override: enable verbose
                    '--log-level', 'DEBUG',  # Override: change log level
                    '--cache-size', '2000',  # Override: change cache size
                ])
                ...
                assert call_args.fuzzy_match.enabled is True  # CLI --fuzzy overrode config False
                assert call_args.semantic_match.enabled is False  # CLI --no-semantic overrode config True
                assert call_args.fuzzy_match.threshold == 80  # CLI --threshold 0.8 overrode config 50
                assert call_args.max_results == 100  # CLI --max-results 100 overrode config 25
                assert call_args.output_format == "table"  # CLI --output table overrode config json
                assert call_args.verbose is True  # CLI --verbose overrode config False
                assert call_args.log_level == "DEBUG"  # CLI --log-level DEBUG overrode config WARNING
                assert call_args.performance.cache_size == 2000  # CLI --cache-size 2000 overrode config 1000
