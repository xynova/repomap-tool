...
from repomap_tool.dependencies import ImportAnalyzer, DependencyGraph, CentralityCalculator, ImpactAnalyzer
...
class TestTreeDependencyIntegration:
    ...
    def test_enhance_entrypoints_with_dependencies(self):
        ...
        discoverer._build_project_dependency_graph.assert_called_once_with("/test/project")
    ...
    def test_extract_file_path_from_location(self):
        ...
        file_path = discoverer._extract_file_path_from_location("src/test.py:123", "/test/project")
        ...
        file_path = discoverer._extract_file_path_from_location("src/test.py", "/test/project")
        expected_path = os.path.normpath("/test/project/src/test.py")
        assert file_path is None  # File doesn't exist
        
        # Test with relative path - should return None since file doesn't exist
        file_path = discoverer._extract_file_path_from_location("test.py:10", "/test/project")
    ...
    def test_lazy_initialization_of_analysis_components(self):
        ...
        discoverer._enhance_entrypoints_with_dependencies.assert_called_once_with([entrypoint], "/test/project")
