...
def test_hybrid_matcher():
    ...
    for query, description in test_cases:
        ...
        for i, (identifier, score, components) in enumerate(matches[:5]):
            ...
            print(f"      Overall: {score:.2f} | Fuzzy: {components['fuzzy']:.2f} | TF-IDF: {components['tfidf']:.2f} | Vector: {components['vector']:.2f} | Context: {components['context']:.2f}")
    ...
    for word, freq in sorted(matcher.word_frequencies.items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f"   '{word}': {freq} occurrences")
    
    # Show IDF values
    print("\nIDF values (higher = more distinctive):")
    for word, idf in sorted(matcher.idf_cache.items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f"   '{word}': {idf:.2f}")
